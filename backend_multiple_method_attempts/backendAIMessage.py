...
@app.route('/backendAIMessage', methods=['POST'])
def run_LLM():
...
# response = client.chat.completions.create(model="gpt-3.5-turbo",
#     messages=[{"role": "user", "content": "based on this" + doc_str + \
...
# asyncio.run(crawler(website_address))
# load from disk

# db_load = Chroma(persist_directory="./chroma_db",embedding_function=OpenAIEmbeddings())
# docs = db_load.similarity_search(query)
# print("db_load saved", docs[0].page_content)
# print("db_load saved", docs[1].page_content)
# print("db_load saved", docs[2].page_content)
# print("db_load saved", docs[3].page_content)
